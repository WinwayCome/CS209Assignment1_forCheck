...
public class OnlineCoursesAnalyzer {
  ...
  public Map<String, Integer> getPtcpCountByInstAndSubject() {
    return courses.stream()
        .map(InstSub_Int::new)
        .collect(
            Collectors.groupingBy(InstSub_Int::getName, Collectors.summingInt(InstSub_Int::getSum)))
        .entrySet()
        .stream()
        .map(InstSub_Int::new)
        .sorted(
            Comparator.comparing(InstSub_Int::getSum, Comparator.reverseOrder())
                .thenComparing(InstSub_Int::getName))
        .collect(
            Collectors.toMap(
                InstSub_Int::getName, InstSub_Int::getSum, (k, v) -> v, LinkedHashMap::new));
  }

  // 3
  public Map<String, List<List<String>>> getCourseListOfInstructor() {
    List<Name_Course> name_courseStream =
        courses.stream()
            .filter(x -> x.getInstructors() != null && x.getTitle() != null)
            .flatMap(
                x ->
                    x.getInstructors().contains(",")
                        ? Arrays.stream(x.getInstructors().split(", ", -1))
                            .map(e -> new Name_Course(e, x.getTitle(), false))
                        : Stream.of(new Name_Course(x.getInstructors(), x.getTitle(), true)))
            .toList();
    return Stream.concat(
            name_courseStream.stream()
                .filter(Name_Course::isSelf)
                .collect(
                    Collectors.groupingBy(
                        Name_Course::getName,
                        Collectors.mapping(Name_Course::getCourse, Collectors.toSet())))
                .entrySet()
                .stream()
                .collect(
                    Collectors.toMap(
                ...
                .stream()
                .collect(
                    Collectors.toMap(Map.Entry::getKey, x -> new InstEr_Course(x.getValue(), true)))
                .entrySet()
                .stream(),
            name_courseStream.stream()
                .filter(x -> !x.isSelf())
                ...
                .collect(
                    Collectors.toMap(
                        Map.Entry::getKey, x -> new InstEr_Course(x.getValue(), false)))
                .entrySet()
                .stream())
        .collect(
            Collectors.toMap(
  ...
  public List<String> searchCourses(
    ...
    return courses.stream()
        .filter(
            x ->
                x.getSubject().matches("(?i).*" + courseSubject + ".*")
                    && x.getPercentAudited() >= percentAudited
                    && x.getTotalHours() <= totalCourseHours)
        .map(Course::getTitle)
        .distinct()
        .sorted(Comparator.naturalOrder())
        .collect(Collectors.toList());
  ...
  public List<String> recommendCourses(int age, int gender, int isBachelorOrHigher) {
    return courses.stream()
        .collect(Collectors.groupingBy(Course::getNumber, Collectors.toSet()))
        .entrySet()
        .stream()
        .map(
            x -> {
              OptionalDouble o_age =
                  x.getValue().stream().mapToDouble(Course::getMedianAge).average();
              OptionalDouble o_male =
                  x.getValue().stream().mapToDouble(Course::getPercentMale).average();
              OptionalDouble o_deg =
                  x.getValue().stream().mapToDouble(Course::getPercentDegree).average();
              return new Avg(
                  x.getKey(),
                  x.getValue().stream()
                      .collect(
                          Collectors.collectingAndThen(
                              Collectors.reducing(
                                  BinaryOperator.maxBy(
                                      Comparator.comparing(Course::getLaunchDate))),
                              y -> y.map(Course::getTitle).orElse(null))),
                  age,
                  gender * 100,
                  isBachelorOrHigher * 100,
                  o_age.isPresent() ? o_age.getAsDouble() : 0D,
                  o_male.isPresent() ? o_male.getAsDouble() : 0D,
                  o_deg.isPresent() ? o_deg.getAsDouble() : 0D);
            })
        .sorted(Comparator.comparing(Avg::getRecommend).thenComparing(Avg::getTitle))
        .map(Avg::getTitle)
        .distinct()
        .limit(10)
        .collect(Collectors.toList());
...
class Avg {
  ...
  public Avg(
    ...
    this.title = title;
    this.recommend =
        Math.pow(age - avg_age, 2) + Math.pow(male - avg_male, 2) + Math.pow(deg - avg_deg, 2);
  }
